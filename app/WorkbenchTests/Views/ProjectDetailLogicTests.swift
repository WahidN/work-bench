...
@Test func aTaskWithNoDueDateGetsNoTag() {
    let rows = ProjectDetailLogic.taskRows(
        todos: [manualTodo(id: 1, projectId: 1, dueAt: nil)],
        project: project(id: 1), projects: [project(id: 1)], today: today
    )
...
@Test func factsCountWhatTheTabLists() {
    ...
    #expect(facts.openTasks == 1, "the unpinned Jira mirror and the done task do not count")
    #expect(facts.openPrs == 1, "a merged PR is not open work")
...
@Test func openWorkListsPullRequestsThenIssues() {
    ...
    #expect(items.map(\.id) == ["pr-1", "ticket-7"])
    #expect(items[0].ref == "atlas#23")
    #expect(items[0].title == "Fix nav")
    #expect(items[1].ref == "ATL-7")
    #expect(items[1].title == "Broken filter")
    #expect(items[0].symbol == TodayLogic.pullRequestSymbol)
    #expect(items[1].symbol == TodayLogic.issueSymbol)
}
